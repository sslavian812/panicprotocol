...
public class TokenManager {
    ...
    /**
     * Узел ждёт соединение в течение TICK, если не дождался, значит,
     * всё плохо, и нужно начать реконфигурацию. Если дождался,
     * вычитывает токен, проверяет его и обновляет свою версию числа ?,
     * после чего пытается передавать токен. Если кто-то пытается соединиться
     * с узлом, пока тот находится в состоянии CONFIG, то, наверное,
     * он не знает, что сейчас идёт реконфигурация, поэтому, на всякий случай,
